---
pagetitle: "Drawing SEMs"

code-fold: true
---

:::: {layout="[60, 40]" layout-valign="center"}

::: {#leftcolumn}
<h2 style = "margin: 0px;"> Drawing Structural Equation Models with Ti*k*Z </h2>
:::

::: {#rightcolumn}
  <img style = "margin: 0px; padding: 0px;"  src="logo/logo.svg">
:::

::::

This section presents **Ti*k*Z** scripts to draw the path diagrams presented here.

A very quick introduction on using **Ti*k*Z** to draw SEMS is [here](Tutorial/Intro/Intro.qmd). There are also sections on:

- [Defining styles](Tutorial/Styles/Styles.qmd)
- [Using mathematical symbols and Greek letters](Tutorial/Symbols/Symbols.qmd)
- [Repeating using loops](Tutorial/Longitudinal/Long.qmd)
- [Repeating whole pictures](Tutorial/Factor/Factor.qmd)

<p>

In the scripts to follow, I use the following **Ti*k*Z** libraries:

- positioning - positioning relative to a node
- quotes - labels as strings on paths
- calc - coordinate calculations
- arrows.meta - arrowhead shape
- shapes - triangle and ellipse
- math - simple mathematical operations  

<p>

and the following **LaTeX** packages:

- tikz
- mathastext - use text font for math font
- newpxtext and newpxmath - upright Greek letters  

<p>

There is a standardised set of shapes and symbols used in the diagrams. The key shapes and symbols are shown below.

![](Intro.svg)

These symbols are defined as styles in `SEMstyles.tex`. As well, `SEMstyles.tex` loads the **Ti*k*Z** libraries and **LaTeX** packages.


```{tikz}
%| code-summary: "Click to see SEMstyles.tex"
%| file: "SEMstyles.tex"
```    
<p>

All that is left for the individual **Ti*k*Z** scripts to do is to position the symbols on the canvas.

The scripts are rendered as pdf files, then converted to svg files using [inkscape](https://inkscape.org/).


#### The publications and their path diagrams

::: {.no-indent}
- Jose, P. (2013). *Doing statistical mediation and moderation*. New York, NY: Guilford Press.
:::

<p>

<details>
  <summary>Mediation model diagram</summary>
  <img src="Jose_2013/Jose_2013.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Jose_2013/Jose_2013.tex"
```

<br>

::: {.no-indent}
- Kurbanoglu, N. & Takunyaci, M. (2021). A structural equation modeling on relationship between self-efficacy, physics laboratory anxiety and attitudes. *Journal of Family, Counseling and Education*, *6*(1), 47-56.
:::

<p>

<details>
  <summary>Mediation model diagram</summary>
  <img src="Kurbanoglu_2021/Kurbanoglu_2021.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Kurbanoglu_2021/Kurbanoglu_2021.tex"
```  

<br>

::: {.no-indent}
- Little, T., Slegers, D., & Card, N. (2006). A non-arbitrary method of identifying and scaling latent variables in SEM and MACS models. *Structural Equation Modeling*, *13*(1), 59-72.
:::

<p>

<details>
  <summary>Model diagram for Reference-Group Method</summary>
  <img src="Little_2006/Scaling1.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Little_2006/Scaling1.tex"
``` 

<p>

<details>
  <summary>Model diagram for Marker-Variable Method</summary>
  <img src="Little_2006/Scaling2.svg">  
</details>

```{tikz}
%| code-summary: "Tikz code"
%| file: "Little_2006/Scaling2.tex"
```  

<p>

<details>
  <summary>Model diagram for Effects-Scaling Method</summary>
  <img src="Little_2006/Scaling3.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Little_2006/Scaling3.tex" 
```   

<br> 

::: {.no-indent}
- Thompson, M., Liu, Y. & Green, S. (2023). Flexible structural equation modeling approaches for analyzing means. In R. Hoyle (Ed.), *Handbook of structural equation modeling* (2nd ed., pp. 385-408). New York, NY: Guilford Press.
:::

<p>

<details>
  <summary>one-way ANOVA model diagram</summary>
  <img src="Green_2023/one_way_ANOVA.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Green_2023/one_way_ANOVA.tex"
```   

<p>

<details>
  <summary>one-way ANCOVA model diagram</summary>
  <img src="Green_2023/one_way_ANCOVA.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Green_2023/one_way_ANCOVA.tex"
```

<p>

<details>
  <summary>two-way ANOVA model diagram</summary>
  <img src="Green_2023/two_way_ANOVA.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Green_2023/two_way_ANOVA.tex"
```

<p>

<details>
  <summary>one-way MANOVA model diagram</summary>
  <img src="Green_2023/one_way_MANOVA.svg">  
</details>

```{tikz}
%| code-summary: "Tikz code"
%| file: "Green_2023/one_way_MANOVA.tex"
```

<p>

<details>
  <summary>one-way LATENT model diagram</summary>
  <img src="Green_2023/one_way_LATENT.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Green_2023/one_way_LATENT.tex"
```

<br> 

::: {.no-indent}
- Thompson, M. & Green, S. (2013). Evaluating between-group differences in latent variable means. In G. Hancock & R. Mueller (Eds.), *Structural equation modeling: A second course* (2nd ed., pp. 163-218). Charlotte, NC: Information Age Publishing.
:::

<p>

<details>
  <summary>One-Group model diagram</summary>
  <img src="Thompson_2013/One_Group.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Thompson_2013/One_Group.tex"
```

<p>

<details>
  <summary>Two-Group model diagram</summary>
  <img src="Thompson_2013/Two_Groups.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Thompson_2013/Two_Groups.tex"
```

<br> 

::: {.no-indent}
- Berry, D., & Willoughby, M. (2016). On the practical interpretability of cross-lagged panel models: Rethinking a developmental workhorse. *Child Development*, *88*(4), 1186-1206.
:::

<p>

<details>
  <summary>Lansford - ARCL model diagram</summary>
  <img src="Berry_2016/ARCL_Lansford.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Berry_2016/ARCL_Lansford.tex"
```

<p>

<details>
  <summary>ARCL model diagram</summary>
  <img src="Berry_2016/ARCL.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Berry_2016/ARCL.tex"
```

<p>

<details>
  <summary>ALT_SR model diagram</summary>
  <img src="Berry_2016/ALT_SR.svg">  
</details> 

```{tikz}
%| code-summary: "Tikz code"
%| file: "Berry_2016/ALT_SR.tex"
```

```{r} 
```